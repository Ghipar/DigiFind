...
export const AuthProvider = ({ children }) => {
    ...
    const register = async (nik, name, gender, address, email, password, phone) => {
        setIsLoading(true);
        try {
    ...
    const sendOTP = async (email) => {
        ...
        try {
            const response = await axios.post(`${BASE_URL}/api/send-otp`,{
                ...
                await AsyncStorage.setItem('userId', String(response.data.data.user_id));
            }).catch(response => {
                console.log('error:', response)
            })
        } catch (error) {
    ...
    return (
        <AuthContext.Provider value={{ register, isLoading, userInfo, login, profile, sendOTP, ressPass }}>
            {children}
